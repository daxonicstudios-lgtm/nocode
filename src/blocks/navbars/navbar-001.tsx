...
export default function Navbar001(props: BlockProps) {
  const {
    theme,
    heading = "Acme",
    logoUrl,
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden items-center gap-8 md:flex">
            {navLinks.map((link) => (
              <a key={link.label} href={link.url} className="text-sm font-medium text-gray-700 transition hover:text-gray-900">
                {link.label}
              </a>
            ))}
            <a
        ...
        {open && (
          <div className="border-t border-gray-200 py-4 md:hidden">
            <div className="flex flex-col gap-3">
              ...
              <a
                href={buttonUrl}
                className="mt-2 rounded-md bg-gray-900 px-4 py-2 text-center text-sm font-semibold text-white"
                style={{ backgroundColor: theme?.primary }}
              >
                {buttonText}
              </a>
